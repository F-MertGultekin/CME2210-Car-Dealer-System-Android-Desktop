...
import vehicle.vehicleIdentities.Technic.Build;
        ...
        try{
            ...
            while ((row = csvReader.readLine()) != null) {
                if(counter!=0) {
                    ...
                    Technic technic = new Technic.Build(data[12], data[14], Integer.parseInt(data[8]),Double.parseDouble(money), gear).buildTechnic();
                    Vehicle motorcycle = VehicleFactory.getVehicle(engine, technic,null,1);
                    ((Motorcycle) motorcycle).setMileage(mileage);
                    motorcycles.add((Motorcycle) motorcycle);
